...
/**
 * Project avatar tile — a rounded square carrying the project's 2-letter
 * code on a stable per-name hue. The basename of a session's cwd is passed
 * in as `name`; initials come from the first two segments split on
 * [-_/\s], and the hue is a hash of the name so the same project always
...
export function TgAvatar({
  ...
  // ponytail: project-initials coloring — hash the name to a stable hue,
  // take first letters of the first two segments. basename(cwd) is passed in.
  let hash = 0;
  for (let i = 0; i < name.length; i++) hash = (hash + name.charCodeAt(i)) | 0;
  const face = _AVATAR_HUES[Math.abs(hash) % _AVATAR_HUES.length];
  const code =
    name.split(/[-_/\s]/).filter(Boolean).slice(0, 2)
      .map((p) => p[0]?.toUpperCase()).join("") || "?";
  return (
    <span className={cn("relative inline-flex items-center justify-center", dims)}>
      <span className={cn("flex items-center justify-center rounded-xl font-semibold", dims, face)}>
        {code}
      </span>
      {active && (
